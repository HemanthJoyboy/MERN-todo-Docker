...
# Confirm all 4 containers are listed here. If one's missing, that
...
### 9.5 auth-service / todo-service can't connect to Neon Postgres

**Symptom:** container starts, but crashes immediately or every DB query
fails with a connection error in `docker logs`.

**What it means:** unlike inter-container calls, this is normal internet
traffic leaving the container — Docker networking isn't involved. Usual
suspects:

1. `DATABASE_URL` in the service's `.env` is wrong, expired, or missing
   `?sslmode=require` (Neon requires SSL).
2. The `.env` file wasn't actually picked up — check the `--env-file .env`
   flag was included in the `docker run` command, or that the path is
   correct relative to where you ran it from.
3. Neon project is paused (Neon's free tier auto-suspends idle databases —
   the first request after idle can be slow, not necessarily broken).

**Fix / verify:**
```bash
docker exec -it auth-service sh
env | grep DATABASE_URL     # confirm the value actually made it into the container
```
If it's empty, the `--env-file` path was wrong or the flag was left off
entirely when the container was started.
